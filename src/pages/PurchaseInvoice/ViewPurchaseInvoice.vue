<template>
  ...
  <q-card class="q-pa-md q-ma-sm">
    <div class="cardTitle cardTitleSticky">Purchase Invoices</div>
    <div class="row items-center">
      <div class="col-12 col-md-4">
        <q-input
          filled
          ref="visitDateRef"
          v-model="invoiceStrDate"
          mask="####-##-##"
          label="Start Date"
    ...
    <div>
      ...
      >
        ...
        <template
          ...
          v-slot:body-cell-arrived="props"
        >
          <q-td :props="props">
            <div class="q-pa-md">
              <q-checkbox
          ...
          <div class="q-pa-xs col-xs-12 col-sm-6 col-md-4">
            <q-card flat bordered>
              ...
              <q-card-section class="text-left">
                <div class="row">
                  <span class="col-12 text-weight-bolder"> Supplier Name </span>
                  <span class="col-12">
                    {{ props.row.name }}
                  </span>
                </div>
                <div class="row">
                  <span class="col-12 text-weight-bolder"> Invoice Date </span>
                  <span class="col-12">
                    {{ props.row.date }}
                  </span>
                </div>
...
const handleUpdate = (row) => {
  $q.dialog({
    ...
    componentProps: {
      componentToRender: PurchaseInvoice,
...
const onApply = async () => {
  for (let i = 0; i < rowsInvoice.value.length; i++) {
    if (!is.deepEqual(origRowInoice.value[i], rowsInvoice.value[i])) {
      ...
      rowsInvoice.value = rowsInvoice.value.filter(
        (value) => value.id !== rowsInvoice.value[i].id
      );
    }
  }
  clone();
